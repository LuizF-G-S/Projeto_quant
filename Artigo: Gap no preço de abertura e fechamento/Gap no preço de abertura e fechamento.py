...
def SML(dados, ano_de_referencia): 
    ...
    dates_now = dados[f'{ano_de_referencia}']
    dates_PL = dates['Patrimônio Líquido']
    dates_prices = dates['Preços Ajustados']
    dates_prices_now = dates_now['Preços Ajustados']
    ...
    dates_prices_now = dates_prices_now.dropna(thresh=258 - limite_nan, axis=1)
    ...
    SML = ((ret_SP + ret_SM + ret_SH)/3)-((ret_HP + ret_HM + ret_HH)/3)
...
def HML(dados, ano_de_referencia): 
    ...
    HML = ((ret_SH + ret_HH)/2)-((ret_HP + ret_SP)/2)
